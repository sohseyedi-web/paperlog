...
const PaperBox = ({ item }) => {
  return (
    <div
      key={item._id}
      className="px-3 flex rounded-md shadow-md w-[90%] mx-auto md:mx-0 md:w-[48%] py-2 border border-indigo-300 "
    >
      <div className="w-1/3">
        <img
      ...
      <div className=" w-2/3 mt-5 flex flex-col justify-between">
        ...
        <div className="flex items-center justify-end gap-x-3">
          <button>
            <RiIcon.RiBookmarkLine size={24} />
          </button>
          <button>
            <RiIcon.RiHeart3Line size={24} />
          </button>
          <Link href={`/paper/${item._id}`}>
            <RiIcon.RiArrowRightLine size={24} />
          </Link>
        </div>
      </div>
    </div>
